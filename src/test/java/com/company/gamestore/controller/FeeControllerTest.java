...
public class FeeControllerTest {
    ...
    public void shouldCreateFee() throws Exception {
        ...
        mockMvc.perform(post("/fees").content(inputJson).contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
    ...
    public void shouldGetAFeeById() throws Exception {
        mockMvc.perform(get("/fees/123")).andDo(print()).andExpect(status().isOk());
    }

    @Test
    public void shouldGetAllFees() throws Exception {
        mockMvc.perform(get("/fees")).andDo(print()).andExpect(status().isOk());
    }

    @Test
    public void deleteFee() throws Exception {
        mockMvc.perform(delete("/fees/23")).andDo(print()).andExpect(status().isNoContent());
    }
}